 ...
 * · 13 whaleSkeleton + in-world credits bag (`FloatingCreditsBag`)
 */
...
export const SALMON_ENV = {
   ...
   */
  whaleSkeleton: true,
  /** Drifting plastic bag credits card — restore ≥13; kill `?aqsalmonkill=credits`. */
  creditsBag: true,
};
 ...
 * rebuilding. Example: `?aqsalmonkill=vault,backdrop,distantSilhouettes,credits`
...
export function getSalmonEnv() {
  ...
  try {
    ...
    if (!raw) return env;
    const killAliases = { whale: 'whaleSkeleton', credits: 'creditsBag' };
 ...
 * · 13 whaleSkeleton · creditsBag (`FloatingCreditsBag`)
 */
export function buildSalmonEnvForScene(themeId) {
  ...
  return {
    ...
    creditsBag: base.creditsBag && rs >= 13,
  };
}
